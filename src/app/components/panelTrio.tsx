  ...
  return (
    <div className='flex justify-center mb-9 mx-10'>
      <div className='flex gap-6 mb-9 w-full max-w-[1280px]'>
        {items.map(item => (
          <div key={item.title} className='group rounded-3xl overflow-hidden h-[796px] w-full max-w-[410px]'>
            <div 
              className='flex self-center w-full bg-center h-full max-h-[502px] bg-cover group-hover:brightness-[90%] transition-all ease-in' 
            ...
            <div className='p-[50px] bg-[#444655] h-full flex flex-col items-center justify-between max-h-[294px]'>
              ...
              <a 
                href={item.url} 
                className='text-black hover:text-[#EEE8A9] border border-[#EEE8A9] text-xl rounded-3xl bg-[#EEE8A9] hover:bg-black px-16 py-2 w-fit mt-10 uppercase'
              >
                Shop Now
              </a>
            </div>
